...
import org.apache.flink.ml.math.{Breeze, Vector, BLAS}
...
/** Multiple linear regression using the ordinary least squares (OLS) estimator.
  ...
  * @example
  *          {{{
  *             val mlr = MultipleLogisticRegression ()
  *               .setIterations(10)
  *               .setStepsize(0.5)
  ...
  * =Parameters=
  *
  *  - [[MultipleLogisticRegression .Iterations]]:
  *  Maximum number of iterations.
  *
  *  - [[MultipleLogisticRegression .Stepsize]]:
  ...
  *  performance.
  *
  *  - [[MultipleLogisticRegression .ConvergenceThreshold]]:
  ...
  *
  */
class MultipleLogisticRegression extends Predictor[MultipleLogisticRegression ] {
  import MultipleLogisticRegression._
  import org.apache.flink.ml._

  // Stores the weights of the linear model after the fitting phase
  var weightsOption: Option[DataSet[WeightVector]] = None

  def setIterations(iterations: Int): MultipleLogisticRegression = {
    parameters.add(Iterations, iterations)
    this
  }

  def setStepsize(stepsize: Double): MultipleLogisticRegression  = {
    parameters.add(Stepsize, stepsize)
    this
  }

  def setConvergenceThreshold(convergenceThreshold: Double): MultipleLogisticRegression  = {
...
//
  ...
  def squaredResidualSum(input: DataSet[LabeledVector]): DataSet[Double] = {
    weightsOption match {
      ...
      case None => {
        throw new RuntimeException("The MultipleLogisticRegression  has not been fitted to the " +
...
object MultipleLogisticRegression  {

  val WEIGHTVECTOR_BROADCAST = "weights_broadcast"

  val lossFunction = GenericLossFunction(SquaredLoss, LogisticPrediction)
...
//  }
  ...
  def apply(): MultipleLogisticRegression  = {
    new MultipleLogisticRegression ()
  }

  // ====================================== Operations =============================================

  /** Trains the linear model to fit the training data. The resulting weight vector is stored in
    * the [[MultipleLogisticRegression ]] instance.
    *
    */
  implicit val fitMLR = new FitOperation[MultipleLogisticRegression , LabeledVector] {
    override def fit(
      instance: MultipleLogisticRegression ,
...
//        .setMiniBatchRate(miniBatchRate)
  ...
  implicit def predictVectors[T <: Vector] = {
    new PredictOperation[MultipleLogisticRegression , WeightVector, T, Double]() {
      override def getModel(self: MultipleLogisticRegression , predictParameters: ParameterMap)
        ...
        self.weightsOption match {
          ...
          case None => {
            throw new RuntimeException("The MultipleLogisticRegression " +
