...
//      .map { x => LabeledVector(x(2), DenseVector(x.take(2).toArray)) }
    ...
    def runAndGetPrec(mbr:Double,lr:Double):Double={
      val mlr = org.apache.flink.ml.regression.MultipleLogisticRegression()
        .setIterations(ceil(100/mbr).toInt)
        .setStepsize(lr)
...
//        .map(x=>x._2/x._1).collect().head
      ...
      val fallout=eval
        .filter(_._1==0).map(x=>(x._1,round(x._2+(0.5-thr))))
        .reduce{(left,right)=>(left._1+right._1,left._2+right._2)}
        .map(x=>x._2/x._1).collect().head
...
//      precision.print()
    ...
    val res =for (
      mbr <- Seq(0.01); lr <- Seq(2) //mbr-nek egesszamszor meg kell lennie 1-ben (kulonben nem annyi, amennyit irtunk, mert ceil())
    ) yield {
      runAndGetPrec(mbr,lr)
    }
